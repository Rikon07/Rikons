...
export default function MobileNav({ open, onClose, profilePic, name, navItems }) {
  return (
    <div className={`fixed inset-0 z-50 bg-black/30 flex flex-col md:hidden transition-opacity duration-300 ${open ? "opacity-100" : "pointer-events-none opacity-0"}`}>
      <div className="bg-white dark:bg-[#18181b] rounded-2xl shadow-xl m-2 mt-4 p-4 flex flex-col gap-4">
        ...
        <div className="bg-white dark:bg-[#18181b] rounded-2xl p-4 flex flex-col gap-3 shadow">
          {navItems.map((item) => {
            ...
            return (
              <a
                key={item.name}
                href={item.link}
                className="flex items-center gap-3 text-gray-700 dark:text-gray-200 text-sm px-2 py-1 rounded-lg hover:bg-gray-100 dark:hover:bg-[#232329]"
                onClick={e => {
                  e.preventDefault();
                  onClose();
